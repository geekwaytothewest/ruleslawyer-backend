...
> **Infrastructure is defined and provisioned by the `geekway-infra` CDK
...
> runtime env/secrets/sizing, or migrate prod, see `geekway-infra`:
...
Changing container env vars or secrets is an **infra change in `geekway-infra`**
...
The workflows **deploy** to infrastructure that `geekway-infra` provisions — they
do **not** create it. Before a deploy can succeed:

- The environment must be stood up via `geekway-infra` (cluster, services, ECR
...
- **Runtime config lives in CDK, not here.** Container env vars, secrets, and
  CPU/memory are defined in `geekway-infra` and are the source of truth. The
