...
`bggId` linking it to its BGG entry, plus a `lastBGGSync` timestamp.
...
Our API license with Board Game Geek requires our users to also use Tabletop.Events for registration.
...
`GET thing?id=<bggId>` → parse XML → update the `Game` with the
[mapped fields](#field-mapping-bgg-thing--game), including an inline cover-art
download. Lookups are by id with **no `&type=boardgame` filter**, so expansions
(`boardgameexpansion`) resolve too. A game with no `bggId` is skipped.
...
| `coverArt`        | Bytes downloaded from `thumbnail` (skipped/deferred in the bulk loop)  |
