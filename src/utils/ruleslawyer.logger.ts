...
export class RuleslawyerLogger implements LoggerService {
	...
	error(message: any, ...optionalParams: any[]) {
		this.logger.error(`${message}; traceId=${this.cls.getId()}`, optionalParams);
	}
	warn(message: any, ...optionalParams: any[]) {
		this.logger.warn(`${message}; traceId=${this.cls.getId()}`, optionalParams);
	}
	debug?(message: any, ...optionalParams: any[]) {
		this.logger.debug(`${message}; traceId=${this.cls.getId()}`, optionalParams);
	}
	verbose?(message: any, ...optionalParams: any[]) {
		this.logger.verbose(`${message}; traceId=${this.cls.getId()}`, optionalParams);
	}
	fatal?(message: any, ...optionalParams: any[]) {
		this.logger.fatal(`${message}; traceId=${this.cls.getId()}`, optionalParams);
	}
	log(message: string) {
