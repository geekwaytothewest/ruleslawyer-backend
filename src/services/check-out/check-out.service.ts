...
export class CheckOutService {
  ...
  async getLongestCheckouts(conventionId: number, ctx: Context) {
    try {
      ...
      this.logger.error(
        `Getting longest checkouts for conventionId=${conventionId}, ex=${ex}`,
      );
  ...
  async getRecentCheckouts(conventionId: number, ctx: Context) {
    try {
      ...
      this.logger.error(
        `Failed to get recent checkouts for conventionId=${conventionId}, ex=${ex}`,
      );
      return Promise.reject(ex);
    }
  }
  ...
  async checkOut(
    ...
    try {
      this.logger.log(
        `Checking out copy with collectionId=${collectionId}, copyBarcode=${copyBarcode}, conventionId=${conventionId}, attendeeBarcode=${attendeeBarcode}, overrideLimit=${overrideLimit}`,
      );
      ...
      if (!copy) {
        this.logger.error(
          `Copy not found with collectionId=${collectionId}, copyBarcode=${copyBarcode}`,
        );
      ...
      if (copy.checkOuts.filter((co) => !co.checkIn).length > 0) {
        this.logger.error(
          `Copy already checked out with collectionId=${collectionId}, copyBarcode=${copyBarcode}`,
        );
        return Promise.reject('already checked out');
      }

      this.logger.log(
        `Getting attendee with checkouts with conventionId=${conventionId}, attendeeBarcode=${attendeeBarcode}`,
      );
      ...
      if (!attendee) {
        this.logger.error(
          `Attendee not found with conventionId=${conventionId}, attendeeBarcode=${attendeeBarcode}`,
        );
      ...
      this.logger.error(
        `Error checking out copy with collectionId=${collectionId}, copyBarcode=${copyBarcode}, conventionId=${conventionId}, attendeeBarcode=${attendeeBarcode}, overrideLimit=${overrideLimit}, ex=${ex}`,
      );
      return Promise.reject(ex);
    }
  }
  ...
  async checkIn(collectionId: number, copyBarcode: string, ctx: Context) {
    try {
      this.logger.log(
        `Checking in copy with collectionId=${collectionId}, copyBarcode=${copyBarcode}`,
      );
      this.logger.log(
        `Getting copy with collectionId=${collectionId}, copyBarcode=${copyBarcode}`,
      );
      const copy = await this.copyService.copyWithCheckouts(
        {
          collectionId_barcode: {
      ...
      this.logger.log(
        `Copy found with collectionId=${collectionId}, copyBarcode=${copyBarcode}, copyId=${copy.id}`,
      );
      ...
      if (checkOuts.length === 0) {
        this.logger.error(
          `Copy is already checked in with collectionId=${collectionId}, copyBarcode=${copyBarcode}, copyId=${copy.id}`,
        );
      ...
      this.logger.log(
        `Checking in copy with collectionId=${collectionId}, copyBarcode=${copyBarcode}, copyId=${copy.id}`,
      );
      ...
      this.logger.error(
        `Failed to check in copy with collectionId=${collectionId}, copyBarcode=${copyBarcode}`,
      );
      return Promise.reject(ex);
    }
  }
  ...
  async submitPrizeEntry(
    ...
    try {
      ...
      if (!checkOut?.checkIn) {
        this.logger.error(
          `Play with checkoutId=${checkOutId} is not checked in`,
        );
      ...
      if (checkOut.submitted) {
        this.logger.error(
          `Play with checkoutId=${checkOutId} is already submitted`,
        );
      ...
      for (const p of players) {
        ...
        if (p.rating && p.rating > 5) {
          this.logger.error(
            `Attempt to submit play with checkoutId=${checkOutId} with invalid rating=${p.rating}`,
          );
      ...
      try {
        this.logger.log(
          `Creating players for checkoutId=${checkOutId} with ${players?.length} players`,
        );
        ...
        this.logger.error(
          `Failed to create players for checkoutId=${checkOutId} with ${players?.length} players`,
        );
      ...
      this.logger.error(
        `Failed to submit prize entry for checkoutId=${checkOutId}, ex=${ex}`,
      );
      return Promise.reject(ex);
    }
  }
  ...
  async getAttendeePrizeEntries(attendeeBadgeNumber: string, ctx: Context) {
    try {
      ...
      this.logger.error(
        `Failed to get attendee prize entries with attendeeBadgeNumber=${attendeeBadgeNumber}, ex=${ex}`,
      );
      return Promise.reject(ex);
    }
  }
